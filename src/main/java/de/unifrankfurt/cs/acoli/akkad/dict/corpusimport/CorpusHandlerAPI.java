...
package de.unifrankfurt.cs.acoli.akkad.dict.corpusimport;
...
public abstract class CorpusHandlerAPI extends DefaultHandler2 {
     ...
     */
    public abstract String cleanWordString(String word,final boolean reformat);
    ...
    public void crossValidation(final Double foldparam,final String corpusfile) throws IOException,ArithmeticException {
        ...
        while((temp=reader.readLine())!=null){
            if(linecounter<partsize*(i+1)){
                crossfold[i]+=this.cleanWordString(temp,true)+System.lineSeparator();
     ...
     * @throws ParserConfigurationException on error
     */
    public abstract DictHandling dictImport(final String corpus,final TestMethod testMethod,final CharTypes sourcelang)throws IOException, SAXException, ParserConfigurationException;
    ...
    public DictHandling generateTestTrainSets(final String corpus,final String signpath,final Double foldOrPercent,final Double start,final TestMethod testMethod,final CharTypes charTypes) throws IOException,ArithmeticException, SAXException, ParserConfigurationException, XMLStreamException {
        ...
        if(checkForDict(corpus,testMethod)){
            dictHandler=this.dictImport(corpus,testMethod,charTypes);
        }else{
    ...
    public void randomSample(Double sampleSize,final String corpusfile) throws IOException, ParserConfigurationException, SAXException {
        ...
        while((temp=reader.readLine())!=null){
            if(sampleKeys.contains(linecounter)){
                testSet+=this.cleanWordString(temp,true)+System.lineSeparator();
            }else {
    ...
    /**
     * Performs a percentage split on the given corpus.
     * @param perc the percentage of the testset
     * @param startline the line to start
     * @param random indicates if the split should be random
     * @param corpusfile the corpusfile to split
     * @param charTypes the chartype to assume
     * @throws IOException
    ...
    /**
     * Transforms a given transliteration to the boundary representation needed for machine learning.
     * @param chars  the transliteration to transform
     * @param chartype the language to use
     * @return  the boundary representation
     */
    public String transliterationToBoundaries(String chars,final CharTypes chartype){
        chars="["+chars+"]";
    ...
    /**
     * Transforms a given transliteration into a text to be exported for classifying.
     * @param text the transliteration
     * @param duplicator
     * @param dicthandler
     * @param countmisses
     * @param segmented
     ...
     * @throws IOException
     */
    public abstract String transliterationToText(String text,Integer duplicator, DictHandling dicthandler,Boolean countmisses,final Boolean segmented) throws IOException;
}
