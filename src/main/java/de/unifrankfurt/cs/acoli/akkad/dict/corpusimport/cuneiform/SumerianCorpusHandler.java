...
import de.unifrankfurt.cs.acoli.akkad.dict.dicthandler.cuneiform.SumerianDictHandler;
import de.unifrankfurt.cs.acoli.akkad.dict.pos.POSTagger;
import de.unifrankfurt.cs.acoli.akkad.dict.pos.cuneiform.SumerianPOSTagger;
...
public class SumerianCorpusHandler extends CuneiCorpusHandler {
    ...
    public DictHandling generateCorpusDictionaryFromFile(final List<String> filepath,final String signpath,final String filename,final boolean wholecorpus,final boolean corpusstr,final TestMethod testMethod) throws IOException, ParserConfigurationException, SAXException {
        ...
        dicthandler.setCharType(CharTypes.SUMERIANCHAR);
            ...
            if(!notakk && !line.isEmpty() && line.substring(0,1).matches("[0-9]")){
                ...
                for(String word:words){
                    ...
                    if(word.contains("{") && word.contains("}") ){
                        ...
                        SumerianChar akkad;
                        if(chara==null){
                            akkad=new SumerianChar(this.cleanWordString(word.substring(word.indexOf("{")+1,word.indexOf("}")),true));
                        ...
                        dicthandler.addWord(akkad, CharTypes.SUMERIANCHAR);
                        //lastchar=akkad;
    ...
    public DictHandling getUtilDictHandler() {
        if(this.utilDictHandler==null){
            this.utilDictHandler=new SumerianDictHandler(new LinkedList<String>());
            try {
