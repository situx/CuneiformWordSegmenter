...
package de.unifrankfurt.cs.acoli.akkad.dict.chars;
...
public abstract class LangChar implements Comparable<LangChar>{
    ...
    public void addFollowingWord(final String following,final String preceding) {
        this.addFollowingWord(following,preceding, false,false,-1);
    }
    ...
    public void addFollowingWord(final String following) {
        this.addFollowingWord(following,"", false,true,-1);
    }
    ...
    public void addFollowingWord(final String following,final Boolean boundary) {
        this.addFollowingWord(following,"", boundary,boundary,-1);
    }
     ...
     * @param following the following word to add
     */
    public void addFollowingWord(final String following,final String preceding,final Boolean boundary,final Integer boundarycase) {
        this.addFollowingWord(following,preceding, boundary,boundary,boundarycase);
    }
     ...
     */
    public void addFollowingWord(final String following,final String preceding, final boolean followingboundary,final boolean precedingboundary,final Integer boundarycase) {
        ...
        switch (boundarycase){
            case 0:
                this.followingChars.get(following).addBoundariesFollowBoundaries();
                break;
            case 1:
                this.followingChars.get(following).addBoundariesFollowContinuations();
                break;
            case 2:
                this.followingChars.get(following).addContinuationFollowsBoundary();
                break;
            case 3:
                this.followingChars.get(following).addContinuationFollowsContinuation();
                break;
            default:
        }
    }
    ...
    public void addTranslation(final String translation,final Locale locale){
        ...
        if(!this.translations.containsKey(locale.toString())){
           this.translations.put(locale.toString(),new TreeMap<Translation, Integer>());
        }
    ...
    public boolean equals(final Object obj) {
        if(obj instanceof LangChar){
            return this.character.equals(((LangChar)obj).character);
        }
        return false;
    }
            ...
            case ANKI:
                ...
                for(Transliteration transliteration:this.transliterations.keySet()){
                    if(!collect.contains(importH.reformatToASCIITranscription(transliteration.getTranscription()))){
                        ...
                        buffer.append("\\");
                        buffer.append("<b>Transliteration: </b>" +transliteration.getTransliteration()/*+"<br><br><b>Transkription:</b> "+importH.reformatToASCIITranscription(transliteration.getTranscription())*/);
    ...
    public String toXML(String startelement){
        ...
        try {
            ...
            for(Transliteration akkadtrans:this.transliterations.keySet()){
                writer.writeCharacters(akkadtrans.toXML() + System.lineSeparator());
            }
            ...
            for(String akkadfollow:followingwords.keySet()){
                ...
                writer.writeAttribute(Tags.PRECEDING.toString(), followingwords.get(akkadfollow).getPreceding().keySet().iterator().next());
