...
public class RuleMethods extends SegmentationMethods implements RuleAPI{
    ...
    public void charSegmentParse(final String filepath,final String destpath,final DictHandling dict,final TransliterationMethod transliterationMethod,final CharTypes chartype,final Boolean transcriptToTranslit,final Boolean corpusstr) throws IOException {
        if(corpusstr){
            this.initParsing(filepath, destpath+"_"+ ClassificationMethod.CHARSEGMENTPARSE.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+ Files.RESULT.toString(),dict, ClassificationMethod.CHARSEGMENTPARSE,transliterationMethod,chartype,transcriptToTranslit,corpusstr);

        }else{
            this.initParsing(filepath, filepath.substring(filepath.lastIndexOf("/")+1,filepath.lastIndexOf('.'))+"_"+ ClassificationMethod.CHARSEGMENTPARSE.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+ Files.RESULT.toString(),dict, ClassificationMethod.CHARSEGMENTPARSE,transliterationMethod,chartype,transcriptToTranslit,corpusstr);
    ...
    private String[] charSegmentParse(LangChar tempchar, final String temp,final DictHandling dicthandler,final TransliterationMethod transliterationMethod,final CharTypes chartype) throws IOException {
        ...
        return new String[]{res,translit};
    }
    ...
    public void matchByAvgWordLength(final String filepath,final String destpath, final DictHandling dicthandler, final TransliterationMethod transliterationMethod,final CharTypes chartype,final Boolean transcriptToTranslit,final Boolean corpusstr) throws IOException {
        if(corpusstr){
            this.initParsing(filepath, destpath+"_"+ ClassificationMethod.AVGWORDLEN.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+Files.RESULT.toString(),dicthandler, ClassificationMethod.AVGWORDLEN,transliterationMethod,chartype,transcriptToTranslit,corpusstr);

        }else{
            this.initParsing(filepath, filepath.substring(filepath.lastIndexOf("/")+1,filepath.lastIndexOf('.'))+"_"+ ClassificationMethod.AVGWORDLEN.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+Files.RESULT.toString(),dicthandler, ClassificationMethod.AVGWORDLEN,transliterationMethod,chartype,transcriptToTranslit,corpusstr);
    ...
    private String[] matchByAvgWordLength(LangChar tempchar,String currentline,DictHandling dicthandler,final TransliterationMethod transliterationMethod,final CharTypes chartype) throws IOException {
        ...
        for(i=0;i<=this.tempstr.length()-avglength;i+=avglength){
            ...
            for(int j=0;j<=tmp.length()-charlength;j+=charlength){
                cuneiresult+=tmp.substring(j,j+charlength);
            }
            cuneiresult+=/*tmp.substring(tmp.length()-charlength,tmp.length())+*/" ";
        }
        ...
        String translit=this.assignTransliteration(cuneiresult.split(" "),dicthandler,transliterationMethod);
        return new String[]{cuneiresult,translit};
    ...
    public void prefixSuffixMatching(final String filepath,final String destpath, final DictHandling dicthandler, final TransliterationMethod transliterationMethod, final CharTypes chartype,final Boolean transcriptToTranslit,final Boolean corpusstr) throws IOException {
        if(corpusstr){
            this.initParsing(filepath, destpath+"_"+ ClassificationMethod.PREFSUFF.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+Files.RESULT.toString(),dicthandler, ClassificationMethod.PREFSUFF,transliterationMethod,chartype,transcriptToTranslit,corpusstr);

        }else{
            this.initParsing(filepath, filepath.substring(filepath.lastIndexOf("/")+1,filepath.lastIndexOf('.'))+"_"+ ClassificationMethod.PREFSUFF.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+Files.RESULT.toString(),dicthandler, ClassificationMethod.PREFSUFF,transliterationMethod,chartype,transcriptToTranslit,corpusstr);
    ...
    private String[] randomSegmentParse(LangChar tempchar,final String currentline,final DictHandling dicthandler,final TransliterationMethod transliterationMethod,final CharTypes chartype) throws IOException {
        ...
        int currentbegin=0;
        String currentstr="",translitresult,cuneiformresult;
        cuneiformresult="";
        ...
        for(int i=0;i<=currentstr.length()-charlength;i+=charlength){
            ...
            if(tempchar!=null){
                ...
                cuneiformresult+=tempchar.getCharacter();
            }
        }
        String translit=this.assignTransliteration(cuneiformresult.split(" "),dicthandler,transliterationMethod);
        return new String[]{cuneiformresult,translit};
    ...
    public void randomSegmentParse(final String filepath,final String destpath,final DictHandling dicthandler,final TransliterationMethod transliterationMethod,final CharTypes chartype,final Boolean transcriptToTranslit,final Boolean corpusstr) throws IOException {
        if(corpusstr){
            this.initParsing(filepath, destpath+"_"+ ClassificationMethod.RANDOMSEGMENTPARSE.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+Files.RESULT.toString(),dicthandler, ClassificationMethod.RANDOMSEGMENTPARSE, transliterationMethod,chartype,transcriptToTranslit,corpusstr);

        }else{
            this.initParsing(filepath, filepath.substring(filepath.lastIndexOf("/")+1,filepath.lastIndexOf('.'))+"_"+ ClassificationMethod.RANDOMSEGMENTPARSE.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+Files.RESULT.toString(),dicthandler, ClassificationMethod.RANDOMSEGMENTPARSE, transliterationMethod,chartype,transcriptToTranslit,corpusstr);
        }
       }
    ...
    public void tangoAlgorithm(final String filepath,final String destpath, final DictHandling dicthandler, final int ngramsize, final TransliterationMethod transliterationMethod, final CharTypes chartype,final Boolean transcriptToTranslit,final Boolean corpusstr) throws IOException {
        if(corpusstr){
            this.initParsing(filepath, destpath+"_"+ ClassificationMethod.TANGO.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+Files.RESULT.toString(),dicthandler, ClassificationMethod.TANGO,transliterationMethod,chartype,transcriptToTranslit,corpusstr);

        }else{
            this.initParsing(filepath, filepath.substring(filepath.lastIndexOf("/")+1,filepath.lastIndexOf('.'))+"_"+ ClassificationMethod.TANGO.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+Files.RESULT.toString(),dicthandler, ClassificationMethod.TANGO,transliterationMethod,chartype,transcriptToTranslit,corpusstr);
    ...
    private String[] tangoAlgorithm(final LangChar tempchar,final String line, final DictHandling dicthandler, int ngramsize,final TransliterationMethod transliterationMethod,CharTypes charTypes){
        ...
        return new String[]{cuneiresult,this.assignTransliteration(this.words.toArray(new String[this.words.size()]),dicthandler,transliterationMethod)};
    }
}
