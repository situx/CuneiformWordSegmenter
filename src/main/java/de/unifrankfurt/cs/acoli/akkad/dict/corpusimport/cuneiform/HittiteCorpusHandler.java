...
import de.unifrankfurt.cs.acoli.akkad.dict.dicthandler.cuneiform.HittiteDictHandler;
 ...
 * HittiteCorpusHandler.
 */
public class HittiteCorpusHandler extends CuneiCorpusHandler {
    /**
     * Constructor for this class.
     * @param stopchars the stopcharrs to consider.
     */
    public HittiteCorpusHandler(List<String> stopchars){
        super(stopchars);
    }
    ...
    /**
     * Generates an Hittite corpusimport out of the source file.
     * @param filepath the filepath to the source file
     * @param wholecorpus if the whole corpus should be parsed
    ...
    public DictHandling generateCorpusDictionaryFromFile(final List<String> filepath,final String signpath,final String filename,final boolean wholecorpus,final boolean corpusstr,final TestMethod testMethod) throws IOException, ParserConfigurationException, SAXException {
        ...
        dicthandler.setCharType(CharTypes.HITTITECHAR);
        ...
        this.reformattedTranslitWriter =new BufferedWriter(new FileWriter(new File(Files.REFORMATTEDDIR+filename+Files.REFORMATTED)));
        this.reformattedBoundaryWriter =new BufferedWriter(new FileWriter(new File(Files.REFORMATTEDDIR.toString()+Files.BOUNDARYDIR.toString()+filename+Files.BOUNDARIES.toString())));
        this.reformattedCuneiWriter =new BufferedWriter(new FileWriter(new File(Files.REFORMATTEDDIR.toString()+Files.CUNEI_SEGMENTEDDIR.toString()+filename)));
        ...
        HittiteChar lastchar,lastlastchar;
        String[] words,chars;
        ...
        while((line=this.corpusReader.readLine())!=null){
            ...
            lastchar=new HittiteChar(" ");
            if(line.isEmpty()){
                continue;
            }
            line=line.toLowerCase();
            if(line.substring(0,1).equals("#") && !line.contains(Tags.HIT.toString())){
                ...
                notakk=true;
            }else if(line.substring(0,1).equals("#") && line.contains(Tags.HIT.toString())){
                notakk=false;
            }
            if(!notakk && !line.isEmpty() && line.substring(0,1).matches("[0-9]")){
                ...
                for(String word:words){
                    ...
                    if(word.contains("{") && word.contains("}") ){
                        ...
                        HittiteChar akkad;
                        if(chara==null){
                            akkad=new HittiteChar(this.cleanWordString(word.substring(word.indexOf("{")+1,word.indexOf("}")),true));
                            //dicthandler.addWord(akkad);
                        }
                        else{
                            akkad=new HittiteChar(chara.getCharacter());
                        }
                        ...
                        dicthandler.addWord(akkad, CharTypes.HITTITECHAR);
                        //lastchar=akkad;
                    ...
                    if(!nocunei && !modword.isEmpty()){
                        HittiteChar akkad=new HittiteChar(cuneiword);
    ...
    public DictHandling getUtilDictHandler() {
        if(this.utilDictHandler==null){
            this.utilDictHandler=new HittiteDictHandler(new LinkedList<String>());
            try {
