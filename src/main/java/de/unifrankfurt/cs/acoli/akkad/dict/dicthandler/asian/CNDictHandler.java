...
public class CNDictHandler extends AsianDictHandler {
	...
	public CNDictHandler(List<String> stopchars){
        super(stopchars,CharTypes.CHINESE,new POSTagger(new TreeMap<String, Color>(),CharTypes.CHINESE.getLocale()));
...
;
    ...
    public void addWord(final LangChar word,final CharTypes charType){
        ...
        if(this.dictionary.containsKey(word.getCharacter())){
                ...
                if(tempchar!=null){
                    ...
                    if(lastword!=null) {
                        if (lastlastword != null) {
                            this.lastword.addFollowingWord(templangchar.getCharacter(), lastlastword.getCharacter(), true,2);
                        } else {
                ...
                for(int i=charlength;i<cuneiword.length()-charlength;i+=charlength){
                    ...
                    if(tempchar!=null){
                        ...
                        if(this.lastword!=null) {
                            if(lastlastword!=null){
                                this.lastword.addFollowingWord(templangchar.getCharacter(),lastlastword.getCharacter(), true,3);
                            }else{
                                this.lastword.addFollowingWord(templangchar.getCharacter(), true);
                            }
                            templangchar.addPrecedingWord(this.lastword.getCharacter());
            ...
            if(lastword!=null){
                if(lastlastword!=null){
                    this.lastword.addFollowingWord(word2.getCharacter(),lastlastword.getCharacter(), true,-1);
                }else{
    ...
    public String getDictTransliteration(final LangChar tempword, final TransliterationMethod transliterationMethod) {
        ...
        switch (transliterationMethod){
            case FIRST:return !tempword.getTransliterationSet().isEmpty()?tempword.getTransliterationSet().iterator().next().getTransliteration():"";
            case MAXPROB: return tempword.getMostProbableWordTransliteration().getTransliteration();
    ...
    public String getDictTransliteration(final PositionableChar tempchar, final TransliterationMethod transliterationMethod) {
        ...
        switch (transliterationMethod){
            ...
            case MAXPROB: return tempchar.getMostProbableWordTransliteration().getTransliteration();
    ...
    public String getNoDictTransliteration(final String word, final TransliterationMethod transliterationMethod) {
        ...
        if(word.length()<chartype.getChar_length()){
            ...
            switch(transliterationMethod){
                case MAXPROB: return "["+this.dictmap.get(word).getMostProbableBeginTransliteration(0).getTransliteration()+"]";
                case RANDOM:
                case FIRST:
    ...
    /**
     * Parses a preformatted dictionary file.
