...
public class DictMethods extends SegmentationMethods implements DictMethodAPI {
    ...
    /**Temporary variables for savin chars.*/
    private String chars="",curchar="",oldchars="";
    /**Indicates if the first character is processed at the moment.*/
     ...
     */
	public DictMethods() throws IOException{
        super();
     ...
     * @throws IOException
     */
    public void breakPointMatching(final String filepath,final String destpath,final DictHandling dicthandler,final TransliterationMethod transliterationMethod,final CharTypes chartype,final Boolean corpusstr) throws IOException {
        if(corpusstr){
            this.initParsing(filepath, destpath+"_"+ ClassificationMethod.BREAKPOINT.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+ Files.RESULT.toString(),dicthandler, ClassificationMethod.BREAKPOINT,transliterationMethod,chartype,corpusstr);
        }else{
            this.initParsing(filepath, filepath.substring(filepath.lastIndexOf("/")+1,filepath.lastIndexOf('.'))+"_"+ ClassificationMethod.BREAKPOINT.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+ Files.RESULT.toString(),dicthandler, ClassificationMethod.BREAKPOINT,transliterationMethod,chartype,corpusstr);
        }
    }

    private void breakPointMatching(LangChar tempchar,final String currentline,final DictHandling dicthandler,final TransliterationMethod transliterationMethod,CharTypes chartype) throws IOException {
        ...
        String translit=this.assignTransliteration(cuneiwords, dicthandler, transliterationMethod);
    ...
    private String[] combinedMaxMatch(LangChar tempchar, final String currentline, final DictHandling dicthandler,final TransliterationMethod transliterationMethod,final CharTypes chartype) throws IOException {
        ...
        if(res.length()==(currentline.length()+resultmap.values().size())){
            ...
            this.translitResultWriter.write(translit);
            this.transcriptResultWriter.write(TranscriptionMethods.translitTotranscript(translit));
            return new String[]{res,this.assignTransliteration(res.split(" "),dicthandler,transliterationMethod)};
        }
        this.cuneiResultWriter.write(res);
        //if(!left)
        //    Collections.reverse(cuneiwords);
    ...
    public void initParsing(final String sourcepath,final String destpath,final DictHandling dicthandler,final ClassificationMethod method,final TransliterationMethod transliterationMethod,final CharTypes chartype,final Boolean corpusstr) throws IOException {
        ...
        while((currentline=this.reader.readLine())!=null){
            ...
            for(String segment:segments) {
                ...
                switch (method) {
                    case BREAKPOINT:
                        this.breakPointMatching(tempchar, currentline, dicthandler, transliterationMethod, chartype);
                        break;
                    case LCUMATCHING:
                        this.lcuMatching(tempchar,currentline,dicthandler,8,transliterationMethod,chartype);
                        break;
                    case MAXMATCHCOMBINED:
                        this.combinedMaxMatch(tempchar,currentline,dicthandler,transliterationMethod,chartype);
                        break;
                    case MINMATCH:
                        this.minMatch(tempchar, currentline, dicthandler, transliterationMethod, chartype,true);
                        break;
                    case MINWCMATCH:
                        this.minWCMatching(tempchar, currentline, dicthandler, transliterationMethod, chartype);
                        break;
                    case MINWCMATCH2:
                        this.minWCMatching2(tempchar, currentline, dicthandler, transliterationMethod, chartype);
                        break;
                    case MAXMATCH:
                        this.maxMatch(tempchar, currentline, dicthandler, true, transliterationMethod, chartype);
                        break;
                    case MAXMATCH2:
                        this.maxMatch(tempchar, currentline.length() > 0 && currentline.length()%2!=0? currentline.substring(1) : currentline, dicthandler, false, transliterationMethod, chartype);
                        break;
                    case MAXENT:
                        this.maxEntropyMatching(tempchar, currentline, dicthandler, 5, transliterationMethod, chartype);
                        break;
                    default:
                }
            }
    ...
    public void lcuMatching(final String filepath, final String destpath,final DictHandling dicthandler,final TransliterationMethod transliterationMethod,CharTypes chartype,final Boolean corpusstr) throws IOException {
        if(corpusstr){
            this.initParsing(filepath, destpath+"_"+ ClassificationMethod.LCUMATCHING.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+Files.RESULT.toString(),dicthandler, ClassificationMethod.LCUMATCHING,transliterationMethod,chartype,corpusstr);

        }else{
            this.initParsing(filepath, filepath.substring(filepath.lastIndexOf("/")+1,filepath.lastIndexOf('.'))+"_"+ ClassificationMethod.LCUMATCHING.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+Files.RESULT.toString(),dicthandler, ClassificationMethod.LCUMATCHING,transliterationMethod,chartype,corpusstr);

        }
    }

    private void lcuMatching(final LangChar tempchar, final String currentline, final DictHandling dicthandler,final Integer ngramvalue,final TransliterationMethod transliterationMethod,final CharTypes chartype) throws IOException {
         int charlength=chartype.getChar_length();
...
;
    ...
    @Override
    public void maxEntropyMatching(final String filepath,final String destpath, final DictHandling dicthandler,final TransliterationMethod transliterationMethod,CharTypes chartype,final Boolean corpusstr) throws IOException {
        if(corpusstr){
            this.initParsing(filepath,destpath,dicthandler, ClassificationMethod.MAXENT,transliterationMethod,chartype,corpusstr);

        }else{
            this.initParsing(filepath,filepath,dicthandler, ClassificationMethod.MAXENT,transliterationMethod,chartype,corpusstr);
     ...
     */
    private void maxEntropyMatching(final LangChar tempchar, final String line, final DictHandling dicthandler,final int ngramvalue,final TransliterationMethod transliterationMethod,final CharTypes chartype){
         String ngram;
    ...
    public void maxMatch(final String filepath, final String destpath,final DictHandling dicthandler, final boolean left, final TransliterationMethod transliterationMethod, final CharTypes chartype,final Boolean corpusstr) throws IOException{
        ...
        this.first=true;
         if(left){
             if(corpusstr){
                 this.initParsing(filepath, destpath+"_"+ ClassificationMethod.MAXMATCH.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+ Files.RESULT.toString(),dicthandler, ClassificationMethod.MAXMATCH,transliterationMethod,chartype,corpusstr);

             }else{
                 this.initParsing(filepath, filepath.substring(filepath.lastIndexOf("/")+1,filepath.lastIndexOf('.'))+"_"+ ClassificationMethod.MAXMATCH.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+ Files.RESULT.toString(),dicthandler, ClassificationMethod.MAXMATCH,transliterationMethod,chartype,corpusstr);
             }
             }else{
                if(corpusstr){
                    this.initParsing(filepath, destpath+"_"+ ClassificationMethod.MAXMATCH2.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+ Files.RESULT.toString(),dicthandler, ClassificationMethod.MAXMATCH2,transliterationMethod,chartype,corpusstr);

                }else{
                    this.initParsing(filepath, filepath.substring(filepath.lastIndexOf("/")+1,filepath.lastIndexOf('.'))+"_"+ ClassificationMethod.MAXMATCH2.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+ Files.RESULT.toString(),dicthandler, ClassificationMethod.MAXMATCH2,transliterationMethod,chartype,corpusstr);
     ...
     * @throws IOException on error
     */
	private void maxMatch(final LangChar tempchar, final String currentline, final DictHandling dict, final boolean left,final TransliterationMethod transliterationMethod,final CharTypes charType) throws IOException {
        this.maxMatchImproved(tempchar, currentline, dict, left, transliterationMethod, charType);
    }

    public void maxMatchCombined(final String filepath,final String destpath, final DictHandling dicthandler, final TransliterationMethod transliterationMethod, final CharTypes chartype,final Boolean corpusstr) throws IOException {
        if(corpusstr){
            this.initParsing(filepath, destpath+"_"+ ClassificationMethod.MAXMATCHCOMBINED.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+Files.RESULT.toString(),dicthandler, ClassificationMethod.MAXMATCHCOMBINED,transliterationMethod,chartype,corpusstr);

        }else{
            this.initParsing(filepath, filepath.substring(filepath.lastIndexOf("/")+1,filepath.lastIndexOf('.'))+"_"+ ClassificationMethod.MAXMATCHCOMBINED.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+Files.RESULT.toString(),dicthandler, ClassificationMethod.MAXMATCHCOMBINED,transliterationMethod,chartype,corpusstr);

        }
        }

    private void maxMatchImproved(final LangChar tempchar,final String currentline, final DictHandling dicthandler,final boolean left, final TransliterationMethod transliterationMethod,final CharTypes chartype) throws IOException {
        String tempwordstr;
        int charlength=chartype.getChar_length();
        ...
        for(String word:cuneiwords){
            this.cuneiResultWriter.write(word+" ");
        }
        //if(!left)
        //    Collections.reverse(cuneiwords);
        String translit=this.assignTransliteration(cuneiwords.toArray(new String[cuneiwords.size()]), dicthandler, transliterationMethod);
        this.translitResultWriter.write(translit);
        this.transcriptResultWriter.write(TranscriptionMethods.translitTotranscript(translit));
     ...
     * @throws IOException
     */
    public void minMatch(final String filepath,final String destpath,final DictHandling dicthandler,final TransliterationMethod transliterationMethod,CharTypes chartype,final Boolean corpusstr) throws IOException {
        if(corpusstr){
            this.initParsing(filepath, destpath+"_"+ ClassificationMethod.MINMATCH.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+Files.RESULT.toString(),dicthandler, ClassificationMethod.MINMATCH,transliterationMethod,chartype,corpusstr);

        }else{
            this.initParsing(filepath, filepath.substring(filepath.lastIndexOf("/")+1,filepath.lastIndexOf('.'))+"_"+ ClassificationMethod.MINMATCH.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+Files.RESULT.toString(),dicthandler, ClassificationMethod.MINMATCH,transliterationMethod,chartype,corpusstr);
     ...
     * @throws IOException
     */
    private void minMatch(LangChar tempchar,final String currentline,final DictHandling dicthandler,final TransliterationMethod transliterationMethod,CharTypes chartype,final Boolean corpusstr) throws IOException {
     ...
     */
    @Override
    public void minWCMatching(final String filepath,final String destpath, final DictHandling dicthandler, final TransliterationMethod transliterationMethod, final CharTypes chartype,final Boolean corpusstr) throws IOException {
        if(corpusstr){
            this.initParsing(filepath, destpath+"_"+ ClassificationMethod.MINWCMATCH.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+ Files.RESULT.toString(),dicthandler, ClassificationMethod.MINWCMATCH,transliterationMethod,chartype,corpusstr);

        }else{
            this.initParsing(filepath, filepath.substring(filepath.lastIndexOf("/")+1,filepath.lastIndexOf('.'))+"_"+ ClassificationMethod.MINWCMATCH.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+ Files.RESULT.toString(),dicthandler, ClassificationMethod.MINWCMATCH,transliterationMethod,chartype,corpusstr);

        }
        }

    private void minWCMatching(LangChar tempword,final String currentline,final DictHandling dicthandler,final TransliterationMethod transliterationMethod,CharTypes chartype) throws IOException{
        String tempwordstr;
        int charlength=chartype.getChar_length();
        Boolean probOrFirst=true;
        System.out.println("Currentstr: "+currentline);
    ...
    public void minWCMatching2(final String filepath,final String destpath, final DictHandling dicthandler, final TransliterationMethod transliterationMethod, final CharTypes chartype,final Boolean corpusstr) throws IOException {
        if(corpusstr){
            this.initParsing(filepath,destpath+"_"+ ClassificationMethod.MINWCMATCH2.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+ Files.RESULT.toString(),dicthandler, ClassificationMethod.MINWCMATCH2,transliterationMethod,chartype,corpusstr);

        }else{
            this.initParsing(filepath, filepath.substring(filepath.lastIndexOf("/")+1,filepath.lastIndexOf('.'))+"_"+ ClassificationMethod.MINWCMATCH2.toString().toLowerCase()+"_"+transliterationMethod.getShortlabel().toLowerCase()+ Files.RESULT.toString(),dicthandler, ClassificationMethod.MINWCMATCH2,transliterationMethod,chartype,corpusstr);

        }
        }

    private void minWCMatching2(LangChar tempword,final String currentline,final DictHandling dicthandler,final TransliterationMethod transliterationMethod,CharTypes chartype) throws IOException{
        String tempwordstr;
        int charlength=chartype.getChar_length();
        Boolean probOrFirst=true;
        System.out.println("Currentstr: "+currentline);
    ...
    public void morfessorSegmenting(final String sourcepath,final String testpath,final CuneiDictHandler dictHandler,boolean modelexists,final TransliterationMethod transliterationMethod,final CharTypes chartype) throws IOException, InterruptedException {
        ...
        while((temp=this.reader.readLine())!=null){
            ...
            this.cuneiResultWriter.write("\n");
            translit=this.assignTransliteration(cuneiwords.toArray(new String[cuneiwords.size()]),dictHandler,transliterationMethod);
            this.transcriptResultWriter.write(TranscriptionMethods.translitTotranscript(translit)+"\n");
            this.translitResultWriter.write(translit+"\n");
        }
        this.translitResultWriter.close();
